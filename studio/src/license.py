...
# ==============================================================================
"""
AXTO STUDIO — License Enforcement Module v1.1 (HARDENED)
================================================================
...
log = logging.getLogger("studio.license")
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO STUDIO — LICENSE GRACE PERIOD EXPIRED             ║\n"
    "║                                                              ║\n"
...
async def validate_license(license_key: str) -> LicenseState:
    ...
    async with _lock:
        ...
        if not _state.key.startswith(PRODUCT_PREFIX + "-"):
            _state.valid = False
            _state.error = f"Invalid license key prefix. Expected {PRODUCT_PREFIX}-XXXX-... for AXTO Studio."
        ...
        try:
            ...
            if not _verify_signature(sig_payload, sig):
                log.critical(
                    "AXTO STUDIO — RESPONSE SIGNATURE INVALID. This response "
            ...
            if not signed_fields or signed_fields.get("license_key") != _state.key:
                log.critical("AXTO STUDIO — SIGNED RESPONSE FIELD MISMATCH. Treating as unreachable.")
            ...
            if not granted:
                ...
                log.error(
                    f"❌ AXTO STUDIO — no verified cache available, no grace "
                    f"granted. Error: {e}"
                )
