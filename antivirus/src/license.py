...
# ==============================================================================
"""
AXTO ANTIVIRUS — License Enforcement Module v2.0
================================================================
...
log = logging.getLogger("antivirus.license")
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO ANTIVIRUS — LICENSE GRACE PERIOD EXPIRED           ║\n"
    "║                                                              ║\n"
...
EXPIRY_BANNER = (
    "\n"
    "⚠️  ═══════════════════════════════════════════════════════════\n"
    "   AXTO ANTIVIRUS LICENSE EXPIRES IN {{days}} DAY{{plural}}\n"
...
class LicenseState:
    ...
    def log_expiry_status(self) -> None:
        ...
        ).replace("ANTIVIRUS", PRODUCT.upper())
        if d == 0:
...
class LicenseValidator:
    """
    Validates AXTO ANTIVIRUS license keys against axto.io.
    ...
    async def validate(self, force: bool = False) -> LicenseState:
        ...
        # ── No license key ────────────────────────────────────────────────────
        if not self._key or self._key.upper().startswith(f"AV-XXXX"):
            state = LicenseState(
                valid   = False,
                ...
                message = (
                    ...
                    f"ANTIVIRUS_LICENSE_KEY env var. "
                    f"Purchase at https://axto.io"
                ),
                last_validated = now,
            )
            log.error(
                f"AXTO ANTIVIRUS — NO LICENSE KEY. "
        ...
        if not self._key.upper().startswith(f"AV-"):
            ...
            state = LicenseState(
                ...
                message     = (
                    f"Key prefix '{actual_prefix}' is not valid for AXTO ANTIVIRUS. "
            ...
            log.critical(
                f"AXTO ANTIVIRUS — LICENSE REJECTED: WRONG PRODUCT | "
                f"Key prefix: {actual_prefix} | Required: GUARD | "
                f"Purchase ANTIVIRUS license: https://axto.io"
            )
            self._info = state; self._last = now
            return state
        ...
        try:
            ...
            if not _verify_signature(_sig_payload, _sig):
                log.critical(
                    "AXTO ANTIVIRUS — RESPONSE SIGNATURE INVALID. "
            ...
            if not _signed_fields or _signed_fields.get("license_key") != self._key:
                log.critical(
                    "AXTO ANTIVIRUS — SIGNED RESPONSE FIELD MISMATCH. "
            ...
            if not state.valid:
                ...
                log.info(
                    f"✅ AXTO ANTIVIRUS license valid | "
        ...
        if cached and cached.get("valid") == "true":
            ...
            if within_grace_window and not_past_real_expiry:
                ...
                log.warning(
                    f"⚠️  AXTO ANTIVIRUS OFFLINE GRACE (verified cache) | "
                    f"{max(0, int((GRACE_SECONDS - (now - issued_ts)) / 60))} min remaining."
                )
                return grace_state
            else:
        ...
        self._info = state; self._last = now
        log.critical(SHUTDOWN_MSG.replace("ANTIVIRUS", PRODUCT.upper()))
        return state
