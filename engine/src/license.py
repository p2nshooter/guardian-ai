...
# ==============================================================================
"""
AXTO GUARDIAN — License Enforcement Module v2.0
================================================================
...
log = logging.getLogger("guardian.license")

# ── Constants ─────────────────────────────────────────────────────────────────
PRODUCT         = "guardian"
PRODUCT_PREFIX  = "GUARD"
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO GUARDIAN — LICENSE GRACE PERIOD EXPIRED           ║\n"
    "║                                                              ║\n"
...
EXPIRY_BANNER = (
    "\n"
    "⚠️  ═══════════════════════════════════════════════════════════\n"
    "   AXTO GUARDIAN — FREE ACCESS ENDS IN {{days}} DAY{{plural}}\n"
...
class LicenseState:
    ...
    def log_expiry_status(self) -> None:
        ...
        ).replace("GUARDIAN", PRODUCT.upper())
        if d == 0:
...
class LicenseValidator:
    """
    Validates AXTO GUARDIAN license keys against axto.io.
    ...
    def __init__(self, config: dict):
        self._key   = (
            ...
            or os.environ.get("GUARDIAN_LICENSE_KEY", "")
    ...
    async def validate(self, force: bool = False) -> LicenseState:
        ...
        # ── No license key ────────────────────────────────────────────────────
        if not self._key or self._key.upper().startswith(f"GUARD-XXXX"):
            state = LicenseState(
                valid   = False,
                ...
                message = (
                    ...
                    f"GUARDIAN_LICENSE_KEY env var. "
                    f"Purchase at https://axto.io"
                ),
                last_validated = now,
            )
            log.error(
                f"AXTO GUARDIAN — NO LICENSE KEY. "
        ...
        if not self._key.upper().startswith(f"GUARD-"):
            ...
            state = LicenseState(
                ...
                message     = (
                    f"Key prefix '{actual_prefix}' is not valid for AXTO GUARDIAN. "
                    f"Required prefix: GUARD-. "
                    f"Purchase correct license at https://axto.io"
                ),
                last_validated = now,
            )
            log.critical(
                f"AXTO GUARDIAN — LICENSE REJECTED: WRONG PRODUCT | "
                f"Key prefix: {actual_prefix} | Required: GUARD | "
                f"Purchase GUARDIAN license: https://axto.io"
            )
            self._info = state; self._last = now
            return state
        ...
        try:
            ...
            if not _verify_signature(_sig_payload, _sig):
                log.critical(
                    "AXTO GUARDIAN — RESPONSE SIGNATURE INVALID. "
            ...
            if not _signed_fields or _signed_fields.get("license_key") != self._key:
                log.critical(
                    "AXTO GUARDIAN — SIGNED RESPONSE FIELD MISMATCH. "
        ...
        if cached and cached.get("valid") == "true":
            ...
            if within_grace_window and not_past_real_expiry:
                ...
                log.warning(
                    f"⚠️  AXTO GUARDIAN OFFLINE GRACE (verified cache) | "
                    f"{max(0, int((GRACE_SECONDS - (now - issued_ts)) / 60))} min remaining."
                )
                return grace_state
            else:
        ...
        self._info = state; self._last = now
        log.critical(SHUTDOWN_MSG.replace("GUARDIAN", PRODUCT.upper()))
        return state
