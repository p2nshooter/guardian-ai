...
# ==============================================================================
"""
AXTO STUDIO GPU — License Enforcement Module v1.1 (HARDENED)
================================================================
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO STUDIO GPU — LICENSE GRACE PERIOD EXPIRED             ║\n"
    "║                                                              ║\n"
    ...
    async with _lock:
        ...
        try:
            ...
            if not _verify_signature(sig_payload, sig):
                log.critical(
                    "AXTO STUDIO GPU — RESPONSE SIGNATURE INVALID. This response "
            ...
            if not signed_fields or signed_fields.get("license_key") != _state.key:
                log.critical("AXTO STUDIO GPU — SIGNED RESPONSE FIELD MISMATCH. Treating as unreachable.")
            ...
            if not granted:
                ...
                log.error(
                    f"❌ AXTO STUDIO GPU — no verified cache available, no grace "
                    f"granted. Error: {e}"
                )
