...
// ── AXTO Legal guide overlay (multi-language) ───────────────────────────────
// Kept separate from the large GUIDES object above so the legal product can be
// localized independently. Falls back to EN when a language is unavailable.
const LEGAL_GUIDES: Record<string, string> = {
  en: `# AXTO Legal — Enterprise AI Legal & Compliance Setup Guide

## Overview
AXTO Legal is a self-hosted AI legal, compliance & regulatory-intelligence platform.
LLM inference runs 100% inside your own infrastructure (BYOK / air-gapped capable),
so no sensitive matter ever leaves your network. 195+ country legal packs,
18 AI workspaces, 50+ compliance frameworks.

## Prerequisites
- Docker 24+ and Docker Compose v2+ (or Python 3.11+ for bare-metal)
- 4+ CPU cores, 8+ GB RAM, 60+ GB disk (more for large corpora)
- AXTO Legal license key (prefix: LEGL-)
- An LLM provider key (OpenAI / Anthropic / Gemini / Groq) OR a local Ollama install for air-gapped use

## 1. Get a License
Purchase at https://axto.io/legal, or — if your key was issued manually by an
admin — you can activate immediately with no portal account required. Your key
looks like LEGL-XXXX-XXXX-XXXX-XXXX.

## 2. Configure legal.env
\`\`\`bash
cp legal.env.example .env
\`\`\`
\`\`\`ini
LEGL_LICENSE_KEY=LEGL-YOUR-KEY-HERE
LEGL_API_KEY=choose-a-strong-key-min-32-chars
# LLM — pick one provider (data never sent to AXTO)
LEGL_LLM_PROVIDER=openai
LEGL_LLM_MODEL=gpt-4o-mini
LEGL_LLM_API_KEY=sk-...
# Air-gapped alternative:
# LEGL_LLM_PROVIDER=ollama
# LEGL_OLLAMA_URL=http://localhost:11434
# LEGL_OLLAMA_MODEL=qwen2.5:7b
LEGL_API_PORT=8096
\`\`\`

## 3. Start
\`\`\`bash
docker compose -f docker-compose.yml up -d legal
# Open http://YOUR_SERVER:8096 — the activation screen appears automatically
\`\`\`

## 4. Dashboard Menus
- **Workspaces**: 18 AI legal workspaces (research, contract review, compliance check, multi-jurisdiction, litigation, AML/KYC, ESG, regulatory monitoring…)
- **Documents**: Upload & analyze contracts/policies; export annotated reports (PDF/DOCX)
- **Search**: Natural-language legal search across enabled country packs
- **Regulatory Updates**: Scheduled pulls of new regulations per framework
- **Frameworks**: Enable/disable the 50+ compliance modules you license
- **Jurisdictions**: Manage active country legal packs (3 / 20 / 195+ by tier)
- **Sources**: Legal data-source health and connector status
- **AI Advisor**: Model/tool recommendation per task
- **Settings → License**: Expiry date, seats, country-pack entitlement, activation status

## 5. License Enforcement
- Validated against axto.io; responses are Ed25519-signed and verified locally
- 4-hour offline grace window if the license server is unreachable
- A disabled (OFF) or expired license blocks API requests until restored
- Manage/renew at https://axto.io/portal

## 6. Troubleshooting
\`\`\`bash
docker compose logs -f legal     # view logs
# "invalid_key_prefix": key must start with LEGL-
# "server_unreachable_no_cache": restore connectivity to axto.io, then restart
# 401 on API: send your LEGL_API_KEY in the X-API-Key header
\`\`\`

## Support
hallo@axto.io · response within 1 business day`,

  id: `# AXTO Legal — Panduan Setup Platform Hukum & Compliance AI

## Ringkasan
AXTO Legal adalah platform AI hukum, compliance & intelijen regulasi yang berjalan
100% di infrastruktur Anda sendiri (BYOK / mendukung air-gapped), sehingga tidak
ada data sensitif yang keluar dari jaringan. 195+ country legal pack, 18 AI
workspace, 50+ framework compliance.

## Persyaratan
- Docker 24+ dan Docker Compose v2+ (atau Python 3.11+ untuk bare-metal)
- Minimum 4 CPU core, 8 GB RAM, 60 GB disk (lebih besar untuk korpus besar)
- License key AXTO Legal (prefix: LEGL-)
- Key LLM provider (OpenAI / Anthropic / Gemini / Groq) ATAU Ollama lokal untuk air-gapped

## 1. Dapatkan Lisensi
Beli di https://axto.io/legal, atau — jika key Anda diterbitkan manual oleh admin —
Anda bisa langsung aktivasi TANPA perlu membuat akun portal. Key Anda berbentuk
LEGL-XXXX-XXXX-XXXX-XXXX.

## 2. Konfigurasi legal.env
\`\`\`bash
cp legal.env.example .env
\`\`\`
\`\`\`ini
LEGL_LICENSE_KEY=LEGL-KEY-ANDA-DI-SINI
LEGL_API_KEY=pilih-key-kuat-min-32-karakter
# LLM — pilih satu provider (data tidak pernah dikirim ke AXTO)
LEGL_LLM_PROVIDER=openai
LEGL_LLM_MODEL=gpt-4o-mini
LEGL_LLM_API_KEY=sk-...
...
\`\`\`bash
docker compose -f docker-compose.yml up -d legal
# Buka http://IP_SERVER:8096 — layar aktivasi muncul otomatis
\`\`\`

## 4. Menu Dashboard
- **Workspaces**: 18 workspace hukum AI (riset, review kontrak, cek compliance, multi-yurisdiksi, litigasi, AML/KYC, ESG, monitoring regulasi…)
- **Documents**: Unggah & analisis kontrak/kebijakan; ekspor laporan beranotasi (PDF/DOCX)
- **Search**: Pencarian hukum bahasa natural di seluruh country pack aktif
- **Regulatory Updates**: Tarik regulasi baru terjadwal per framework
- **Frameworks**: Aktif/nonaktifkan 50+ modul compliance yang Anda lisensikan
- **Jurisdictions**: Kelola country legal pack aktif (3 / 20 / 195+ sesuai tier)
- **Sources**: Status kesehatan sumber data hukum & connector
- **AI Advisor**: Rekomendasi model/tool per tugas
- **Settings → License**: Tanggal expiry, seat, hak country pack, status aktivasi

## 5. Penegakan Lisensi
- Divalidasi ke axto.io; respons ditandatangani Ed25519 & diverifikasi lokal
- Grace period 4 jam jika server lisensi tidak terjangkau
- Lisensi yang OFF atau kedaluwarsa memblokir request API hingga dipulihkan
- Kelola/perpanjang di https://axto.io/portal

## 6. Troubleshooting
\`\`\`bash
docker compose logs -f legal     # lihat log
# "invalid_key_prefix": key harus diawali LEGL-
# "server_unreachable_no_cache": pulihkan koneksi ke axto.io, lalu restart
# 401 di API: kirim LEGL_API_KEY Anda di header X-API-Key
\`\`\`
...
export async function GET(req: NextRequest) {
  ...
  const content = GUIDE_PRODUCTS.includes(product)
    ...
    : (product === "legal"
        ? (LEGAL_GUIDES[lang] || LEGAL_GUIDES["en"])
...
<div class="footer"><p>© ${new Date().getFullYear()} AXTO Platform &nbsp;·&nbsp; <a href="mailto:hallo@axto.io">hallo@axto.io</a> &nbsp;·&nbsp; <a href="https://axto.io/portal">axto.io/portal</a></p><p style="margin-top:6px;color:#cbd5e1">Confidential — Licensed customers only</p></div>
</div></body></html>`;
