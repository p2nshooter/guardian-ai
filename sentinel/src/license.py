...
# ==============================================================================
"""
AXTO SENTINEL — License Enforcement Module v2.0
================================================================
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO SENTINEL — LICENSE GRACE PERIOD EXPIRED           ║\n"
    "║                                                              ║\n"
...
EXPIRY_BANNER = (
    "\n"
    "⚠️  ═══════════════════════════════════════════════════════════\n"
    "   AXTO SENTINEL LICENSE EXPIRES IN {{days}} DAY{{plural}}\n"
...
class LicenseState:
    ...
    def log_expiry_status(self) -> None:
        ...
        ).replace("SENTINEL", PRODUCT.upper())
        if d == 0:
...
class LicenseValidator:
    """
    Validates AXTO SENTINEL license keys against axto.io.
    ...
    def __init__(self, config: dict):
        self._key   = (
            ...
            or os.environ.get("SENTINEL_LICENSE_KEY", "")
    ...
    async def validate(self, force: bool = False) -> LicenseState:
        ...
        # ── No license key ────────────────────────────────────────────────────
        if not self._key or self._key.upper().startswith(f"SNTL-XXXX"):
            state = LicenseState(
                valid   = False,
                ...
                message = (
                    ...
                    f"SENTINEL_LICENSE_KEY env var. "
                    f"Purchase at https://axto.io"
                ),
                last_validated = now,
            )
            log.error(
                f"AXTO SENTINEL — NO LICENSE KEY. "
        ...
        if not self._key.upper().startswith(f"SNTL-"):
            ...
            state = LicenseState(
                ...
                message     = (
                    f"Key prefix '{actual_prefix}' is not valid for AXTO SENTINEL. "
                    f"Required prefix: SNTL-. "
                    f"Purchase correct license at https://axto.io"
                ),
                last_validated = now,
            )
            log.critical(
                f"AXTO SENTINEL — LICENSE REJECTED: WRONG PRODUCT | "
                f"Key prefix: {actual_prefix} | Required: SNTL | "
                f"Purchase SENTINEL license: https://axto.io"
            )
            self._info = state; self._last = now
            return state
        ...
        try:
            ...
            if not _verify_signature(_sig_payload, _sig):
                log.critical(
                    "AXTO SENTINEL — RESPONSE SIGNATURE INVALID. "
            ...
            if not _signed_fields or _signed_fields.get("license_key") != self._key:
                log.critical(
                    "AXTO SENTINEL — SIGNED RESPONSE FIELD MISMATCH. "
        ...
        if cached and cached.get("valid") == "true":
            ...
            if within_grace_window and not_past_real_expiry:
                ...
                log.warning(
                    f"⚠️  AXTO SENTINEL OFFLINE GRACE (verified cache) | "
                    f"{max(0, int((GRACE_SECONDS - (now - issued_ts)) / 60))} min remaining."
                )
                return grace_state
            else:
        ...
        self._info = state; self._last = now
        log.critical(SHUTDOWN_MSG.replace("SENTINEL", PRODUCT.upper()))
        return state
