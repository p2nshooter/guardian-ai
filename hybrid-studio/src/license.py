...
# ==============================================================================
"""
AXTO HYBRID STUDIO — License Enforcement Module v1.1 (HARDENED)
================================================================
...
log = logging.getLogger("hybrid_studio.license")

PRODUCT          = "hybrid-studio"      # this app's own identity (cache file, logs)
STUDIO_MODULE    = "hybrid"             # this app's entry in STUDIO_PLANS[].studios (lib/studio.ts)
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO HYBRID STUDIO — LICENSE GRACE PERIOD EXPIRED             ║\n"
    "║                                                              ║\n"
    ...
    async with _lock:
        ...
        if not _state.key.startswith(PRODUCT_PREFIX + "-"):
            ...
            _state.error = (
                ...
                f"— this is your AXTO Studio license (Hybrid Studio is one of the "
                f"sub-apps it unlocks, alongside AI Studio and GPU Studio)."
            )
        ...
        try:
            ...
            if not _verify_signature(sig_payload, sig):
                log.critical(
                    "AXTO HYBRID STUDIO — RESPONSE SIGNATURE INVALID. This response "
            ...
            if not signed_fields or signed_fields.get("license_key") != _state.key:
                log.critical("AXTO HYBRID STUDIO — SIGNED RESPONSE FIELD MISMATCH. Treating as unreachable.")
            ...
            if resp.status_code == 200 and data.get("valid"):
                ...
                if STUDIO_MODULE not in studios:
                    _state.valid = False
                    _state.error = (
                        "Your AXTO Studio plan does not include Hybrid Studio. "
            ...
            if not granted:
                ...
                log.error(
                    f"❌ AXTO HYBRID STUDIO — no verified cache available, no grace "
                    f"granted. Error: {e}"
                )
