...
# ==============================================================================
"""
AXTO EDGE — License Enforcement Module v2.0
================================================================
...
log = logging.getLogger("edge.license")
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO EDGE — LICENSE GRACE PERIOD EXPIRED           ║\n"
    "║                                                              ║\n"
...
EXPIRY_BANNER = (
    "\n"
    "⚠️  ═══════════════════════════════════════════════════════════\n"
    "   AXTO EDGE LICENSE EXPIRES IN {{days}} DAY{{plural}}\n"
...
class LicenseValidator:
    """
    Validates AXTO EDGE license keys against axto.io.
    ...
    def __init__(self, config: dict):
        self._key   = (
            ...
            or os.environ.get("EDGE_LICENSE_KEY", "")
    ...
    async def validate(self, force: bool = False) -> LicenseState:
        ...
        # ── No license key ────────────────────────────────────────────────────
        if not self._key or self._key.upper().startswith(f"EDGE-XXXX"):
            state = LicenseState(
                valid   = False,
                ...
                message = (
                    ...
                    f"EDGE_LICENSE_KEY env var. "
                    f"Purchase at https://axto.io"
                ),
                last_validated = now,
            )
            log.error(
                f"AXTO EDGE — NO LICENSE KEY. "
        ...
        if not self._key.upper().startswith(f"EDGE-"):
            ...
            state = LicenseState(
                ...
                message     = (
                    f"Key prefix '{actual_prefix}' is not valid for AXTO EDGE. "
                    f"Required prefix: EDGE-. "
                    f"Purchase correct license at https://axto.io"
                ),
                last_validated = now,
            )
            log.critical(
                f"AXTO EDGE — LICENSE REJECTED: WRONG PRODUCT | "
                f"Key prefix: {actual_prefix} | Required: EDGE | "
                f"Purchase EDGE license: https://axto.io"
            )
            self._info = state; self._last = now
            return state
        ...
        try:
            ...
            if not _verify_signature(_sig_payload, _sig):
                log.critical(
                    "AXTO EDGE — RESPONSE SIGNATURE INVALID. "
            ...
            if not _signed_fields or _signed_fields.get("license_key") != self._key:
                log.critical(
                    "AXTO EDGE — SIGNED RESPONSE FIELD MISMATCH. "
        ...
        if cached and cached.get("valid") == "true":
            ...
            if within_grace_window and not_past_real_expiry:
                ...
                log.warning(
                    f"⚠️  AXTO EDGE OFFLINE GRACE (verified cache) | "
                    f"{max(0, int((GRACE_SECONDS - (now - issued_ts)) / 60))} min remaining."
                )
                return grace_state
            else:
        ...
        self._info = state; self._last = now
        log.critical(SHUTDOWN_MSG.replace("EDGE", PRODUCT.upper()))
        return state
