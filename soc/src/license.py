...
# ==============================================================================
"""
AXTO SOC — License Enforcement Module v2.0
================================================================
...
log = logging.getLogger("soc.license")
...
SHUTDOWN_MSG    = (
    "\n"
    "╔══════════════════════════════════════════════════════════════╗\n"
    "║     AXTO SOC — LICENSE GRACE PERIOD EXPIRED           ║\n"
    "║                                                              ║\n"
...
class LicenseValidator:
    """
    Validates AXTO SOC license keys against axto.io.
    ...
    async def validate(self, force: bool = False) -> LicenseState:
        ...
        # ── No license key ────────────────────────────────────────────────────
        if not self._key or self._key.upper().startswith(f"SOC-XXXX"):
            state = LicenseState(
                valid   = False,
                ...
                message = (
                    ...
                    f"SOC_LICENSE_KEY env var. "
                    f"Purchase at https://axto.io"
                ),
                last_validated = now,
            )
            log.error(
                f"AXTO SOC — NO LICENSE KEY. "
        ...
        if not self._key.upper().startswith(f"SOC-"):
            ...
            state = LicenseState(
                ...
                message     = (
                    f"Key prefix '{actual_prefix}' is not valid for AXTO SOC. "
                    f"Required prefix: SOC-. "
                    f"Purchase correct license at https://axto.io"
                ),
                last_validated = now,
            )
            log.critical(
                f"AXTO SOC — LICENSE REJECTED: WRONG PRODUCT | "
                f"Key prefix: {actual_prefix} | Required: SOC | "
                f"Purchase SOC license: https://axto.io"
            )
            self._info = state; self._last = now
            return state
        ...
        try:
            ...
            if not _verify_signature(_sig_payload, _sig):
                log.critical(
                    "AXTO SOC — RESPONSE SIGNATURE INVALID. "
            ...
            if not _signed_fields or _signed_fields.get("license_key") != self._key:
                log.critical(
                    "AXTO SOC — SIGNED RESPONSE FIELD MISMATCH. "
            ...
            if not state.valid:
                if state.reason == "product_mismatch":
                    log.critical(
                        f"AXTO SOC LICENSE — PRODUCT MISMATCH | "
                    ...
                    log.critical(
                        f"AXTO SOC — MACHINE ID MISMATCH | "
        ...
        if cached and cached.get("valid") == "true":
            ...
            if within_grace_window and not_past_real_expiry:
                ...
                log.warning(
                    f"⚠️  AXTO SOC OFFLINE GRACE (verified cache) | "
                    f"{max(0, int((GRACE_SECONDS - (now - issued_ts)) / 60))} min remaining."
                )
                return grace_state
            else:
        ...
        self._info = state; self._last = now
        log.critical(SHUTDOWN_MSG.replace("SOC", PRODUCT.upper()))
        return state
